...
void init_tables(uint64_t limit) {
...
uint64_t meissel_pi(uint64_t x) {
...
uint64_t find_nth_prime_fast(uint64_t target_n) {
    if (target_n <= num_primes) {
        return primes[target_n];
    }

    // 1. Asymptotic approximation with safety margin
    ...
    // 2. Compute exact pi(x_start) via Meissel-Lehmer (runs in ~30 ms)
    uint64_t pi_start = meissel_pi(x_start);
    while (pi_start >= target_n) {
        x_start -= margin;
        pi_start = meissel_pi(x_start);
    }

    uint64_t rank_needed = target_n - pi_start;

    // 3. Sieve the window from x_start upwards using 64 KiB segments
    uint64_t low_k = (x_start - 1) / 2 + 1; // odd number index
    uint64_t seg_size = 65536ULL;
    uint8_t *seg = aligned_alloc(64, seg_size);

    uint64_t sqrt_max = (uint64_t)sqrt(est + 500000.0) + 1000;
    uint32_t bprimes_count = pi_table[sqrt_max] - 1; // skip 2
...
int main(int argc, char **argv) {
    ...
    init_tables(LIMIT);
    uint64_t ans = find_nth_prime_fast(target_n);

    clock_gettime(CLOCK_MONOTONIC, &ts2);
    double elapsed = (ts2.tv_sec - ts1.tv_sec) + (ts2.tv_nsec - ts1.tv_nsec) * 1e-9;

    printf("====================================================================\n");
    printf("     Ultra-Fast Combinatorial Sieve (Meissel-Lehmer + AVX-512)     \n");
    printf("====================================================================\n");
