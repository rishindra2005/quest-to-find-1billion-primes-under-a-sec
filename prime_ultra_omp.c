...
void init_compressed_tables_parallel(uint64_t max_val) {
    uint32_t words = max_val / 64 + 1;
    uint32_t sqrt_max = (uint32_t)sqrt(max_val) + 1;
    uint8_t *base_comp = calloc(sqrt_max + 1, 1);
    uint32_t *base_primes = malloc((sqrt_max + 1) * sizeof(uint32_t));
    uint32_t num_bp = 0;

    for (uint32_t p = 3; p <= sqrt_max; p += 2) {
        if (!base_comp[p]) {
            base_primes[num_bp++] = p;
            for (uint32_t j = p * p; j <= sqrt_max; j += 2 * p) base_comp[j] = 1;
        }
    }
    free(base_comp);
    ...
    #pragma omp parallel
    {
        ...
        uint32_t w_start = (uint64_t)words * tid / nthreads;
        uint32_t w_end = (uint64_t)words * (tid + 1) / nthreads;

        for (uint32_t w = w_start; w < w_end; w++) {
            prime_bits[w] = 0xAAAAAAAAAAAAAAAAULL;
        }
        if (tid == 0) {
            prime_bits[0] &= ~((1ULL << 0) | (1ULL << 1));
            prime_bits[0] |= (1ULL << 2);
        }

        uint64_t chunk_low = (uint64_t)w_start * 64;
        uint64_t chunk_high = (uint64_t)w_end * 64 - 1;
        if (chunk_high > max_val) chunk_high = max_val;
        ...
        for (uint32_t i = 0; i < num_bp; i++) {
            ...
            if (p * p >= chunk_low) {
                ...
                uint64_t rem = chunk_low % p;
                start = (rem == 0) ? chunk_low : (chunk_low + (p - rem));
            }
            if (start % 2 == 0) start += p;
            uint64_t step = 2 * p;
            for (uint64_t j = start; j <= chunk_high; j += step) {
...
static __thread HashEntry hash_table[HASH_SIZE];
...
int64_t phi_rec_omp(uint64_t x, uint32_t a) {
    if (a == 4) {
    ...
    int64_t res = phi_rec_omp(x, a - 1) - phi_rec_omp(x / primes_list[a], a - 1);
    hash_table[h].x = x;
    hash_table[h].a = a;
...
uint64_t meissel_pi_ultra_omp(uint64_t x) {
    ...
    uint32_t c = 35;
    int64_t phi_val = phi_rec_omp(x, c);
    int64_t sub_sum = 0;

    #pragma omp parallel for schedule(dynamic, 4) reduction(+:sub_sum)
    for (uint32_t i = c + 1; i <= a; i++) {
        sub_sum += phi_rec_omp(x / primes_list[i], i - 1);
    }
    phi_val -= sub_sum;
...
uint64_t find_nth_prime_ultra_omp(uint64_t target_n) {
    ...
    uint64_t pi_start = meissel_pi_ultra_omp(x_start);
    while (pi_start >= target_n) {
        x_start -= margin;
        pi_start = meissel_pi_ultra_omp(x_start);
    }
    ...
    uint32_t bprimes_count = fast_pi_inline(sqrt_max) - 1;
...
int main(int argc, char **argv) {
    ...
    init_compressed_tables_parallel(MAX_VAL);
    uint64_t ans = find_nth_prime_ultra_omp(target_n);

    clock_gettime(CLOCK_MONOTONIC, &ts2);
    double elapsed = (ts2.tv_sec - ts1.tv_sec) + (ts2.tv_nsec - ts1.tv_nsec) * 1e-9;

    printf("====================================================================\n");
    printf("     RAW ASSEMBLY MULTI-CORE ENGINE (Zen 5 AVX-512 + BMI2)         \n");
    printf("====================================================================\n");
